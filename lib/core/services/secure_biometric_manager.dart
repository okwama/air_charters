...
class SecureBiometricManager {
  static final SecureBiometricManager _instance =
  ...
  Future<bool> isBiometricAvailable() async {
    try {
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Biometric available: $isAvailable, Device supported: $isDeviceSupported',
            name: 'biometric');
      }
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Error checking biometric availability: $e',
            name: 'biometric');
      }
      return false;
    }
  ...
  Future<List<BiometricType>> getAvailableBiometrics() async {
    try {
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Available biometrics: $biometrics',
            name: 'biometric');
      }
      return biometrics;
    } catch (e) {
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error getting biometric types: $e',
            name: 'biometric');
      }
      return [];
    }
  ...
  Future<bool> isBiometricEnabled() async {
    try {
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Biometric enabled: $isEnabled',
            name: 'biometric');
      }
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Error checking biometric enabled status: $e',
            name: 'biometric');
      }
      return false;
    }
  ...
  Future<bool> isBiometricDataValid() async {
    try {
      final String? expiryStr =
          await _secureStorage.read(key: _biometricExpiryKey);
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Biometric data valid: $isValid, Expires: $expiry',
            name: 'biometric');
      }
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Error checking biometric data validity: $e',
            name: 'biometric');
      }
      return false;
    }
  ...
  Future<void> enableBiometric(AuthModel authData) async {
    try {
      ...
      // ✅ STEP 1: Authenticate with biometric FIRST to verify it's the user
      ...
      if (!authenticated) {
        throw AuthException('Biometric authentication cancelled or failed');
      }

      // ✅ STEP 2: Now store the biometric data (user is verified)
      // Create secure biometric identifier (not full auth data)
      final String biometricId = _generateBiometricId();
      final DateTime expiry =
          DateTime.now().add(Duration(days: AppConfig.biometricDataExpiryDays));
      ...
      await _secureStorage.write(
          key: _biometricExpiryKey, value: expiry.toIso8601String());

      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Biometric authentication enabled successfully',
            name: 'biometric');
      }
    } catch (e) {
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error enabling biometric: $e',
            name: 'biometric');
      }
  ...
  Future<void> disableBiometric() async {
    try {
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Biometric authentication disabled and data cleared',
            name: 'biometric');
      }
    } catch (e) {
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error disabling biometric: $e',
            name: 'biometric');
      }
  ...
  Future<Map<String, dynamic>?> authenticateWithBiometric() async {
    try {
      ...
      if (!await isBiometricDataValid()) {
        throw AuthException(
            'Biometric data has expired. Please re-enable biometric authentication.');
      }
      ...
      if (isAuthenticated) {
        ...
        if (userData != null) {
          if (kDebugMode) {
            dev.log(
                'SecureBiometricManager: Biometric authentication successful',
                name: 'biometric');
          }
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Biometric authentication error: $e',
            name: 'biometric');
      }
  ...
  Future<String> _encryptBiometricData(Map<String, dynamic> data) async {
    try {
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error encrypting biometric data: $e',
            name: 'biometric');
      }
  ...
  Future<Map<String, dynamic>?> _decryptBiometricData() async {
    try {
      final String? encryptedData =
          await _secureStorage.read(key: _biometricDataKey);
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error decrypting biometric data: $e',
            name: 'biometric');
      }
      return null;
    }
  ...
  Future<void> clearAllBiometricData() async {
    try {
      ...
      if (kDebugMode) {
        dev.log('SecureBiometricManager: All biometric data cleared',
            name: 'biometric');
      }
    } catch (e) {
      if (kDebugMode) {
        dev.log('SecureBiometricManager: Error clearing biometric data: $e',
            name: 'biometric');
      }
    }
  }
  ...
  Future<bool> needsBiometricRefresh() async {
    try {
      ...
      if (kDebugMode) {
        dev.log(
            'SecureBiometricManager: Error checking biometric refresh need: $e',
            name: 'biometric');
      }
      return true;
    }
